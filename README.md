...
- **列插件**：右侧「插件管理 · N 个已启用」面板列出当前 profile 安装的每个用户插件：状态灯（绿=启用 / 灰=禁用）、插件名、版本号；面板会**异步查询 outdated**，有新版插件显示「可升级 ↑」徽标与「升级」按钮，无新版的不出现升级按钮。
...
- **升级**：**仅当 outdated 确认该插件确有新版时**才显示「升级」按钮（走 dsh plugin ... outdated），点击时仍会二次校验，有新版才执行 dsh plugin ... update <pkg>；已是最新则明确提示「无需升级」，不会无脑硬升。

...
//!
//! Pure-std module (no tauri/gpui types): everything the desktop shell needs to
...
pub enum KernelStatus {
    ...
    /// Intentionally stopped (not crashing).
    Stopped,
}
...
pub struct Kernel {
    ...
    /// Directory holding per-profile log files (`dsh-<profile>.log`).
    log_dir: PathBuf,
...
impl Kernel {
    pub fn new(config: KernelConfig, log_dir: PathBuf) -> Self {
        let (exec, exec_error) = match resolve_dsh() {
            Ok(e) => (Some(e), None),
            Err(e) => (None, Some(e.to_string())),
        };
        let log_path = log_dir.join(format!("dsh-{}.log", config.profile));
    ...
    /// Switch the active profile and repoint the per-profile log file.
    pub fn set_profile(&mut self, profile: &str) {
        self.config.profile = profile.to_string();
        self.log_path = self.log_dir.join(format!("dsh-{profile}.log"));
        let mut w = self.log_writer.lock();
        *w = LogWriter::open(&self.log_path);
        let _ = w.write_line(&format!("=== WhaleNest profile switch: {profile} (new log) ==="));
    }
    ...
    /// Read the trailing `n` lines of the current profile's log.
    pub fn log_tail(&self, n: usize) -> String {
        read_log_tail(&self.log_path, n)
    }
    ...
    pub fn spawn(&mut self) -> Result<(), String> {
        ...
        let mut command = {
            ...
            let mut c = build_windows_command(exec, &self.config.profile, &port_str);
            c.creation_flags(CREATE_NO_WINDOW);
            c
        };
        #[cfg(not(windows))]
        let mut command = build_unix_command(exec, &self.config.profile, &port_str);
...
#[cfg(windows)]
fn build_windows_command(exec: &DshExec, profile: &str, port: &str) -> Command {
    ...
    if shim.extension().and_then(|e| e.to_str()) == Some("ps1") {
        ...
        c.args(["/C", shim_str.as_str(), "--profile", profile, "--no-open", "--port", port]);
        c
    }
}

#[cfg(not(windows))]
fn build_unix_command(exec: &DshExec, profile: &str, port: &str) -> Command {
